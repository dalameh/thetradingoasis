...
type SetupFormProps = {
  ...
  onCancel: () => void;
  onSave: (data: Omit<SetupFormState, "rules"> & { rules: string[] }) => void;
};
...
function rulesReducer(state: string[], action: RuleAction) {
  switch (action.type) {
    case 'update': {
...
export default function SetupForm({ initialData, onCancel, onSave }: SetupFormProps) {
  ...
  const [rules, dispatchRules] = useReducer(rulesReducer, state.rules);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onCancel}>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4 max-h-[80vh] overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-transparent">
          <div className="flex items-center mb-4">
            ...
            <Button variant="ghost" size="icon" onClick={onCancel} className="w-10 flex items-center justify-center text-gray-600 hover:text-gray-900">
              <X className="h-5 w-5" />
            </Button>
          </div>
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              ...
              <Select
                ...
                onChange={(opt) => setState(prev => ({ ...prev, type: opt?.value || '' }))}
                options={typeOptions}
                placeholder="Select type"
                styles={{ control: (base) => ({ ...base, minHeight: 40, height: 31, fontSize: 14 }) }}
              />
            </div>
            <div className="flex-1">
              <label className="block text-sm font-medium text-gray-900 mb-1">
              ...
              <Select
                ...
                onChange={(opt) => setState(prev => ({ ...prev, market: opt?.value || '' }))}
                options={biasOptions}
          ...
          <div>
            ...
            <button type="button" onClick={addRule} className="text-blue-600 hover:text-blue-800 text-xs">+ Add Rule</button>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-900 mb-1">Market Conditions</label>
            <textarea
              value={state.conditions}
              onChange={(e) => setState(prev => ({ ...prev, conditions: e.target.value }))}
          ...
          <button type="submit" className="w-full py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
...
const RuleInput = React.memo(({ value, index, onChange, onRemove, removable }: RuleInputProps) => (
  <div className="flex gap-2 mb-2 items-center">
    ...
    />
    {removable && (
      <button type="button" onClick={() => onRemove(index)} className="px-2 py-1 text-red-600 hover:bg-red-50 rounded text-sm">✖</button>
    )}
  </div>
));