...
export default function Dashboard() {
  ...
  // --- pack main widgets into rows with horizontal and vertical spans ---
  ...
  const toggleWidgetFor = (section: Section, widgetId: string) => {
    ...
    // 🟢 simulate packing with the new widget
    ...
    const placeWidgetInRows = (widgetId: string, spanIn: number, rowSpanIn: number = 1) => {
      ...
      // Instead of blindly creating a new row:
      if (testRows.length + rowSpan > MAX_MAIN_ROWS) {
        return false; // 🚫 not enough rows left for this widget
      }

      // Safe to create new rows
      for (let rr = 0; rr < rowSpan; rr++) {
